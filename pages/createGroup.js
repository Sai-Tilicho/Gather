...
import { getDataFromDb, setDataToDb } from '@/firbase';
...
export default function CreateGroup() {
    ...
    const [timeStamp, setTimeStamp] = useState(new Date()); // Store timestamp as a Date object
    const [imageSrc, setImageSrc] = useState('');
    const [error, setError] = useState('');
    const [ contacts, setContacts] = useState("")
    const [existanceData, setExistanceData] = useState("")
    ...
    useEffect(() => {
        const getUsersDataFromDB = () => {
            getDataFromDb('users', (userData) => {
                const userIdsWithTrueStatus = Object.keys(userData).filter((userId) => {
                    return userData[userId].status === "true";
                });
                getDataFromDb('userContactNumbers/' + `${userIdsWithTrueStatus}`, (contactData) => {
                    try {
                        console.log("object", contactData);
                        const trueContactKeys = Object.keys(contactData).filter(key => {
                        ...
                        if (trueContactKeys.length > 0) {
                            ...
                            console.log("Selected Contact Details:", selectedContactDetails);
    ...
    const handleUpload = () => {
        if (groupName !== '' && description !== '') {
            ...
            if (imgChange?.[0]?.originFileObj) {
                ...
                uploadBytes(imageRef, imgChange[0]?.originFileObj)
                    .then((snapshot) => {
        ...
        try {
            getDataFromDb('group', (data) => {
                ...
                if (!data) {
                    ...
                    const groupExists = existingGroups.filter(groupKey => {
    ...
    return (
        <div className='createGroupPage'>
            <div className='createGroupDiv'>
                <IoIosArrowBack className='groupBackArrow' onClick={() => router.push("/dashboard")} />
                <p className='createGroupTitle'>Create Group</p>
            </div>
            <div>
                <div className='upload_container'>
                    <ImgCrop rotationSlider>
                        <Upload fileList={imgChange}
                            onPreview={onPreview}
                            onChange={onChange}
                            maxCount={1}
            ...
            <Form className='grpForm'>
                <Input placeholder='Enter Group Name' className='groupInput' onChange={handleGroupName} maxLength={20} minLength={1} />
                {error && <Alert type="error" message={error} />}

                <Input placeholder='Enter Description' className='groupInput' onChange={handleDescription} maxLength={20} minLength={1} />
                {error && <Alert type="error" message={error} />}
                <Button className='createGrpButton' onClick={handleUpload}>Create Group</Button>

            </Form>
            {!error && contextHolder}

            {!error && <div className='grpImgDiv'>
                <Image
